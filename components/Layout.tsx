  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#fcfcfc] overflow-hidden font-medium">
      ...
      <div className="md:hidden bg-gray-100 text-black p-4 flex justify-between items-center shadow-md border-b border-gray-200">
        ...
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="w-10 h-10 flex items-center justify-center bg-[#e2006a] text-white rounded-xl shadow-lg shadow-[#e2006a]/20 antialiased active:scale-95 transition-transform">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 z-50 w-72 bg-gray-100 text-gray-800 transition-all duration-500 ease-in-out border-l border-gray-200 antialiased
        md:relative md:right-0
        ${isSidebarOpen ? 'right-0' : '-right-72'}
      `}>
